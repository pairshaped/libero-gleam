...
echo "== Generating benchmark Libero artifacts =="
...

{
  ...
  echo "Method:"
  echo
  echo "- Fixture project generates artifacts from explicit Libero seeds, matching the current Rally-style API."
  echo "- The benchmark owns a tiny \`bench_requests.RequestMsg\` type so request decode can be measured without Libero handler scanning or generated dispatch."
  echo "- ETF helper modules were generated through Libero's public generator APIs so the benchmark can call \`generated@libero_wire\` without compiling any framework dispatch module."
  echo "- BEAM server encode rows include benchmark response pre-encoders plus wire frame encoding."
