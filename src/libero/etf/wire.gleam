...
@external(javascript, "./wire_ffi.mjs", "encode_value")
...
@external(erlang, "libero_etf_ffi", "decode")
@external(javascript, "./wire_ffi.mjs", "decode_value")
...
@external(erlang, "libero_etf_ffi", "decode_safe")
@external(javascript, "./wire_ffi.mjs", "decode_safe")
fn ffi_decode_safe(data: BitArray) -> Result(a, DecodeError) {
  let _ = data
  panic as "libero/etf/wire.ffi_decode_safe: external is missing for this target. This indicates a libero packaging bug; the function should be resolved by the @external attributes."
}
...
@external(erlang, "libero_etf_ffi", "decode_typed")
@external(javascript, "./wire_ffi.mjs", "decodeTypedWire")
fn ffi_decode_typed(
  data: BitArray,
  decoder_name: String,
) -> Result(a, DecodeError) {
  let _ = data
  let _ = decoder_name
  panic as "libero/etf/wire.ffi_decode_typed: external is missing for this target."
}
...
pub fn decode_request(
  ...
  ffi_decode_request(data)
}

// nolint: avoid_panic, discarded_result -- Erlang-only @external; JS fallback is unreachable
@external(erlang, "libero_etf_wire_ffi", "decode_request")
fn ffi_decode_request(
  data: BitArray,
) -> Result(#(String, Int, Dynamic), DecodeError) {
  let _ = data
...
@external(erlang, "libero_etf_wire_ffi", "decode_response_frame")
@external(javascript, "./wire_ffi.mjs", "decode_response_frame")
fn ffi_decode_response_frame(
  data: BitArray,
) -> Result(#(Int, Dynamic), DecodeError) {
  let _ = data
  panic as "libero/etf/wire.ffi_decode_response_frame: external is missing for this target."
}

@external(erlang, "libero_etf_wire_ffi", "decode_push_frame")
@external(javascript, "./wire_ffi.mjs", "decode_push_frame")
fn ffi_decode_push_frame(
  data: BitArray,
) -> Result(#(String, Dynamic), DecodeError) {
  let _ = data
  panic as "libero/etf/wire.ffi_decode_push_frame: external is missing for this target."
}
