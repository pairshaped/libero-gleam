...
Cases = [
  ...
  {"echo_item/basic", 55, {server_echo_item, 7, <<"wrench">>, 12.5, true}},
  ...
  {"echo_with_floats/whole", 58, {server_echo_with_floats, 2.0, 3.0, <<"whole">>}},
  ...
  {"echo_nested_record/basic", 62, {server_echo_nested_record, [Enc(Item), Enc(Item2)], {some, Enc(Item)}, [EncStatus(pending), EncStatus(active), EncStatus(cancelled)], #{<<"one">> => Enc(Item), <<"two">> => Enc(Item2)}}},
  {"echo_typed_err/validation_failed", 63, {server_echo_typed_err, 7, <<"wrench">>, 12.5, true}},
  {"echo_item_list_data/basic", 71, {server_echo_item_list_data, [Enc(Item), Enc(Item2)]}},
  {"echo_item_summary_data/basic", 72, {server_echo_item_summary_data, [Enc(Item)], 42, 1}},
  {"echo_form_prefill/basic", 73, {server_echo_form_prefill, {some, Enc(Item)}, EncStatus(pending)}},
  {"echo_nested_envelope/basic", 74, {server_echo_nested_envelope, EncItemListData({item_list_data, [Item]}), {some, <<"hello">>}}},
  {"echo_dict_and_list_envelope/basic", 75, {server_echo_dict_and_list_envelope, #{<<"one">> => Enc(Item)}, [Enc(Item2)]}},
  {"echo_types_tag/basic", 76, {server_echo_types_tag, <<"sale">>, <<"red">>}},
  {"echo_collision_tag/basic", 77, {server_echo_collision_tag, <<"promo">>}},
